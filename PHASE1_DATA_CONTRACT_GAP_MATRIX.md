...
H-005 and H-006 are preregistered with implemented feature modules (H-005 is
engine-ready; H-006 is blocked on a cross-sectional evaluator), and
...
| H-005 Overnight-Intraday Return Decomposition | Daily OHLCV **including opens** for SPY/QQQ/IWM/EFA/EEM/TLT/GLD plus ^VIX, 2010–2021; 13 overnight/intraday features; 7-fold expanding walk-forward (1260/252/252, purge/embargo 5). | **EXECUTED (CANDIDATE).** `features/overnight_intraday.py`, engine feature-panel branch (now accepts the real-data `^VIX` symbol), `configs/h005_overnight_intraday_trial*.yaml`, frozen protocols `artifacts/h005_trial{1,2}/h005_protocol.json`. Trial `20260915T160008Z_283db198b22dc6aa` passed all 14 frozen gates. | **None for the inspected window. Remaining requirement: an untouched locked window (e.g. 2022–2026) for the `ROBUST_OOS` decision; budget 5/10 trials.** |
| H-006 Factor Exposure Mean Reversion | 17-ETF cross-sectional weekly mean-reversion portfolio: 5 immutable features, Wednesday rebalance, gross ≤ 1.0, net ±40%. | 5 features registered and computable (multi-asset feature path exists); no cross-sectional portfolio/walk-forward evaluator — the scalar engine ranks nothing. | Implement the panel evaluator before Trial 1. |
...
| H-007 Cross-Sectional Quality-Minus-Junk Low-Turnover Core | Cross-sectional quality/fundamentals factor with low-turnover core allocation. | Ledger-only; needs fundamentals data plus the panel evaluator. | Decide the fundamentals data source, or close the preregistration. |
