...
H-005 is executable but has no valid engine evidence yet; H-006 is blocked on a
cross-sectional evaluator; H-004/H-007/H-008 are ledger-only and not
executable.
...
| H-005 Overnight-Intraday Return Decomposition | Daily OHLCV **including opens** for SPY/QQQ/IWM/EFA/EEM/TLT/GLD plus ^VIX, 2010–2021; 13 overnight/intraday features; 7-fold expanding walk-forward (1260/252/252, purge/embargo 5) | Feature module, registry specs, engine feature-panel branch, and configs implemented. Configs omit ^VIX (so `vix_regime` is dropped); Trials 1–4 were standalone-script runs outside the protocol (PRELIMINARY only). | Executable — add ^VIX to the configs and run trials 5–10 through `run_research_pipeline` |
