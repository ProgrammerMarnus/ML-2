...
| H-005 Overnight-Intraday Return Decomposition | Daily OHLCV **including opens** for SPY/QQQ/IWM/EFA/EEM/TLT/GLD plus ^VIX, 2010–2021; 13 overnight/intraday features; 7-fold expanding walk-forward (1260/252/252, purge/embargo 5). | Implemented: `features/overnight_intraday.py`, engine feature-panel branch, `configs/h005_overnight_intraday_trial*.yaml`. Configs omit ^VIX, so `vix_regime` is dropped until they are updated. Trials 1–4 were standalone-script runs outside the protocol. | Add ^VIX to the configs, then run trials 5–10 through `run_research_pipeline`. |
